...
// critique.jsonl + screenshots. Edit the template sections below to restyle.
...
export function buildReport(runDir) {
  ...
  for (const e of journal) if (e.screenshot && !byShot[e.screenshot]) { byShot[e.screenshot] = { screen: e.screen, drive: [], critique: [], verdict: '' }; order.push(e.screenshot); }
  ...
  const head = `<!doctype html><meta charset=utf8><title>FitTrack — autobot report</title>
<style>
 ...
 .screen img{width:200px;border:1px solid #eee;border-radius:8px}
 ...
 .screen h3{margin:0 0 4px}
  ...
  const header = `<header>
 <h1>FitTrack — autobot visual-QA report</h1>
 ...
 <div class=pills>
  ...
  const html = head + header + `<main>${screens}</main>`;
...
if (!runDir || !existsSync(join(runDir, 'journal.jsonl'))) runDir = latestRun(process.env.BUNDLE); // arg may be a bundle, not a dir
